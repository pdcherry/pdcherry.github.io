...
The `optBlock` function from the `AlgDesign` package maximizing $|X'X|$, the determinant of the product of the inverse of the design matrix and the design matrix, a process called *D-optimization*. Further explanation is beyond the scope fo this tutorial, but is available in copious detail in the documentation of the `AlgDesign` package.
