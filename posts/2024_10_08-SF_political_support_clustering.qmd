...
Cluster 3 is fairly dominated by the "No comment" non-endorsements among the SF local ballot measures, but feature prominent "Yes" support for K and L, which contrasts with the varying support for both in clusters 1 and 2. This makes some sense due to the non-partisan interests of measure K (Permanently Closing the Upper Great Highway to Private Vehicles to Establish a Public) and L (Additional Business Tax on Transportation Network Companies and Autonomous Vehicle Businesses to Fund Public Transportation).
