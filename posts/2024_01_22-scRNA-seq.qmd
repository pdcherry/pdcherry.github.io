...
koala_umap_markers_10_per_nest_go_ano
```
```{r}
...
koala_umap_markers_10_per_nest <- koala_umap_markers_10_per_cluster %>%
  ...
  nest() %>%
  mutate(data = as.list(map(data, ~.x[[1]] )))
koala_umap_markers_10_per_nest
```
...
```{r, eval = FALSE}
# in case training doesn't work this round
# mmBrain_classifier <- readRDS("2023_05_25-mmBrain_trained_classified.Rds")
```
...
classified_umap_plot <- ggplot(koala_features_classified_df) +
  geom_point(aes(x = UMAP_1, y = UMAP_2, color = cluster_ext_type)) +
  theme_bw()
