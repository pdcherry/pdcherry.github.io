...
***need better dscription here***

As Searle, _et al._ write, there is a marginal mean for every variable (and every level of factor variables) in a data set, and a sufficiently defined model (be it a linear model, glm, _etc_.) will allow for those marginal means to be estimated.

But I just used "marginal" in the definition of "estimated marginal means." So what does marginal mean? To distill it down, a marginal mean is the effect size that a particular variable contributes, all else being equal. (The all else being equal part comes from accounting for all other _significant_ effects in the model.)
...
Our first task is to come up with a good model. Constructing good models is equal parts art and science, but I won't labor too much over the details; any ready is encouraged to seek more in-depth guidance on model construction and evaluation. I will construct and view some models and settle on one of them:
```{r, warning = FALSE}
...
::: {.callout-note}
We could have used `1/conc` as the response variable, but `emmeans` provides an equivalent `inverse()` function that will prove more advantageous later.
:::
...
```{r}
tidy(EMM.percent) %>%
  ggplot(aes(y = estimate, x = percent)) +
  geom_col(width = 1) +
